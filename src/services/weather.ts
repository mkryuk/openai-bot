...
export async function getWeatherForecast(
  ...
  try {
    ...
    return `Forecast for ${location} on ${date}: ${forecast.weather[0].description}, 
    Temperature ${forecast.main.temp}°C. Feels like ${forecast.main.feels_like}°C.
    Humidity ${forecast.main.humidity}%, Pressure ${forecast.main.pressure}hPa.
    Wind speed ${forecast.wind.speed}m/s, Visibility ${forecast.visibility}m.`;
  } catch (error) {
